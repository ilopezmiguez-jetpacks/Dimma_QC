...
export const QCDataProvider = ({ children }) => {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch Reports (We filter in memory or rely on equipment IDs)
  ...
  const addQCReport = async (reportData) => {
    ...
    // History Calculation
    const reportsForLotAndLevel = qcReports.filter(r => 
      r.equipmentId === reportData.equipmentId && 
      r.lotNumber === reportData.lotNumber && 
      r.level === reportData.level
    );
    ...
    try {
        ...
        setQcReports([...qcReports, formattedReport]);
