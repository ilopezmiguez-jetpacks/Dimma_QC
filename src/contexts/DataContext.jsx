...
export const DataProvider = ({ children }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const isAdmin = user.profile?.role === 'admin' || user.user_metadata?.role === 'admin';
      const userLabId = user.profile?.laboratory_id;

      // 1. Fetch Patients (filtered by lab for non-admins)
      ...
      } else if (!isAdmin && (!patientsData || patientsData.length === 0)) {
        setPatients([]);
        setResults([]);
        setParameters([]);
        return;
  ...
  const addPatient = async (patientData, order) => {
    try {
      const patientToInsert = {
        ...
        status: 'active',
        laboratory_id: user.profile?.laboratory_id
      };
